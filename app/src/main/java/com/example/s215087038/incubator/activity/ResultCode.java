...
public class ResultCode {
    ...
    public String pairCode(String deviceName, long ret) {
        int currentRet = 0;
        if (ret != 0) {
            return deviceName + " " + this.context.getResources().getString(R.string.add_device_success);
        }
        String result;
        if (ret == 0) {
        ...
        switch (currentRet) {
            case COMM_INIT_FAILED /*-203*/:
                result = failString + " " + this.context.getResources().getString(R.string.communication_initialization_failed);
                break;
            case COMMAND_NO_COMMAND /*-13*/:
                result = failString + " " + this.context.getResources().getString(R.string.command_error_no_this_command);
                break;
            case COMMAND_EXECUTION_ERROR /*-12*/:
                result = failString + " " + this.context.getResources().getString(R.string.command_execution_error);
                break;
            case DATA_PARAMETER_ERROR /*-11*/:
                result = failString + " " + this.context.getResources().getString(R.string.data_parameter_error);
                break;
            case DATA_ERROR /*-10*/:
                result = failString + " " + this.context.getResources().getString(R.string.data_error_cannot_resolve);
                break;
            case NEED_CACHE /*-3*/:
                result = failString + " " + this.context.getResources().getString(R.string.need_cache);
                break;
            case COMMAND_NOT_RESPOND /*-2*/:
                result = failString + " " + this.context.getResources().getString(R.string.command_no_respond);
                break;
            case COMMAND_FAILED /*-1*/:
                result = failString + " " + this.context.getResources().getString(R.string.command_failed);
                break;
            default:
                result = failString + " " + this.context.getResources().getString(R.string.add_device_fail);
                break;
        }
        return result;
    }
